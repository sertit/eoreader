...
class MaxarProduct(OpticalProduct):
    """
    ...
    def get_band_paths(self, band_list: list, resolution: float = None) -> dict:
        ...
        if not self.ortho_path:
            if self.product_type in [
                MaxarProductType.SEN,
                MaxarProductType.PRJ,
            ]:
    ...
    def open_mask(self, mask_str: str) -> gpd.GeoDataFrame:
        """
        Open Maxar mask (GML files stored in MASKS) as `gpd.GeoDataFrame`.
        ...
        if mask_path.is_file():
            ...
            # Sometimes the GML mask lacks crs (why ?)
                ...
                in [
                    MaxarProductType.ORT,
                    MaxarProductType.MOS,
                ]
            ):
                with rasterio.open(str(self._get_dimap_path())) as dim_dst:
