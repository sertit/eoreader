...
class S2TheiaProduct(OpticalProduct):
        ...
        for band in band_list:  # Get clean band path
            ...
            if clean_band.is_file():
                ...
                band_paths[band] = self._glob(
                    f"*FRE_B{self.bands[band].id}.tif", as_rio_path=True
                )
        return band_paths
    ...
    def _get_mask_path(self, mask_id: str, res_id: str = "R1") -> AnyPathType:
        ...
        return self._glob(f"*MASKS/*{mask_id}_{res_id}.tif", as_rio_path=True)
    ...
    def _get_condensed_name(self) -> str:
        """
        Get S2THEIA condensed name ({date}_S2THEIA_{tile]_{product_type}).
